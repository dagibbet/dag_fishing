...
-- RegisterNetEvent('dag_fishing:client:UseFishingRod')
-- AddEventHandler('dag_fishing:client:UseFishingRod', function(item)
	-- local ped = PlayerPedId()
	-- SetCurrentPedWeapon(ped, 0xABA87754, true)
-- end)
                ...
                if FISHING_GET_MINIGAME_STATE() == 7 then
                    ...
                    if GetGameTimer() >= nextAttTime then
                        ...
                        if probabilidadePuxar > 0.8 or probabilidadePuxar < 0.2 then -- soltar linha
                            ...
                            local r = exports["dag_fishing"]:VERTICAL_PROBE(x, y,  z, 1)
                            local valid, height = r[1], r[2]
														
                        -- import from ptfx on dag_fishing c# version
                ...
                if FISHING_GET_MINIGAME_STATE() == 12 then
                    if IsControlJustPressed(0, GetHashKey("INPUT_ATTACK")) then
                        if fishing then 
						    ...
						    TriggerServerEvent("dag_fishing:FishToInventory", fishModel)
						    TriggerServerEvent("dag_fishing:discord_keep2", fishModel, fishing_data.fish.weight)
...
function GET_TASK_FISHING_DATA()
    local r = exports["dag_fishing"]:GET_TASK_FISHING_DATA_EXTRA()
    hasMinigameOn = r[1]
    local outAsInt = r[2]
...
function SET_TASK_FISHING_DATA()
    if fishing_minigame_struct.f_0 ~= nil then
        exports["dag_fishing"]:SET_TASK_FISHING_DATA_EXTRA(fishing_minigame_struct)
    end
end
